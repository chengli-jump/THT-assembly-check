...
import yaml
import sys
sys.path.append('F:\\siemens\\tht-assembly-check\\tht-assembly-check')  #加入路径，添加目录
...
def _compare(image, golden_sample_image):
    ...
    # image_saturated = cv2.cvtColor(image_saturated,cv2.COLOR_BGR2GRAY)
    # golden_sample_image_saturated = cv2.cvtColor(golden_sample_image_saturated,cv2.COLOR_BGR2GRAY)
    # diff_img = cv2.absdiff(image_saturated, golden_sample_image_saturated)
...
def _find_pcb_area_of_image1(image_raw,is_golden_sample=True):
...
if __name__ =='__main__':
    ...
    file_directory = 'F:\\siemens\\tht-assembly-check\\tht-assembly-check\config\\7-21.yaml'
    with open(file_directory, 'r', encoding='utf-8') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
    path1 = r'raw_images\Picture1.png'

    # path1 = r'golden_images\sample1.png'
    ...
    # canvas = _find_pcb_area_of_image1(raw_image)
